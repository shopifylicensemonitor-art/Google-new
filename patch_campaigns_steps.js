...
if (!code.includes('const [workflowSteps, setWorkflowSteps]')) {
  ...
  // 2. Include workflowSteps in the create payload
  code = code.replace(
  ...
  // 3. Clear workflow steps on save
  code = code.replace(
    "setVariations([{ subject: '', body_html: '' }]);",
    "setVariations([{ subject: '', body_html: '' }]);\n        setWorkflowSteps([]);"
  );
  
  // 4. Add UI for Workflow Steps below the Plain Text Fallback
  const insertionMarker = '<div className="pt-4 flex items-center justify-end gap-3 border-t border-border/40">';
  const workflowUI = `
  ...
  if (code.includes(insertionMarker)) {
    code = code.replace(insertionMarker, workflowUI + "\n" + insertionMarker);
    fs.writeFileSync('gfg-main/src/pages/Campaigns.tsx', code);
    console.log('patched Campaigns.tsx with workflow builder');
  } else {
