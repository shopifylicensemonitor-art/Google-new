...
const feedEnd = '{/* Recent Replies Sidebar Panel (1 Span) */}';

if (code.includes(feedStart) && code.includes(feedEnd)) {
  const parts = code.split(feedStart);
  const secondPart = parts[1];
  const feedEndIndex = secondPart.indexOf(feedEnd);
  
  if (feedEndIndex !== -1) {
    const sectionToRemove = secondPart.substring(0, feedEndIndex);
    ...
    const newCode = parts[0] + '<div className="p-6 flex-1 flex flex-col gap-6 overflow-y-auto max-h-[400px]">' + replacement + feedEnd + secondPart.substring(feedEndIndex + feedEnd.length);
    fs.writeFileSync('gfg-main/src/pages/Dashboard.tsx', newCode);
    console.log('patched Dashboard.tsx feed successfully');
  } else {
    console.log('Could not find feed end');
  }
} else {
  console.log('Could not find feed start or end');
}
